# InvoiceFlow Agent - Staging Environment Infrastructure
# This configuration sets up the core AWS infrastructure for the staging environment
...
terraform {
  ...
  backend "s3" {
    bucket         = "invoiceflow-terraform-state-staging"
    key            = "staging/terraform.tfstate"
    region         = "us-west-2"
    encrypt        = true
    dynamodb_table = "invoiceflow-terraform-locks-staging"
  }
}
